...
public sealed class RolesController : BaseApiController
{
    ...
    public RolesController(IUnitOfWork uow, ISender sender, IMapper mapper)
    {
        _uow = uow;
        _sender = sender;
        _mapper = mapper;
    ...
    {
        var items = await _uow.Roles.GetAllAsync(ct);
        return Ok(_mapper.Map<IReadOnlyList<RoleDto>>(items));
    }
    ...
    public async Task<IActionResult> GetPaged([FromQuery] int page = 1, [FromQuery] int pageSize = 10, [FromQuery] string? search = null, CancellationToken ct = default)
    {
        ...
        var items = await _uow.Roles.GetPagedAsync(page, pageSize, search, ct);
        var total = await _uow.Roles.CountAsync(search, ct);
        var mapped = _mapper.Map<IReadOnlyList<RoleDto>>(items);
    ...
    {
        var item = await _uow.Roles.GetByIdAsync(id, ct);
        if (item is null) return NotFound();
        return Ok(_mapper.Map<RoleDto>(item));
    }
    ...
    public async Task<IActionResult> Create([FromBody] CreateRoleRequest request, CancellationToken ct)
    {
        ...
        var item = await _uow.Roles.GetByIdAsync(id, ct);
        if (item is null) return NotFound();
        return CreatedAtAction(nameof(GetById), new { id }, _mapper.Map<RoleDto>(item));
    }
    ...
    public async Task<IActionResult> Update(Guid id, [FromBody] UpdateRoleRequest request, CancellationToken ct)
    {
        ...
        var command = _mapper.Map<UpdateRole>(request) with { Id = id };
