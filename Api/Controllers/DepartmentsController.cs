...
public sealed class DepartmentsController : BaseApiController
{
    ...
    public DepartmentsController(IUnitOfWork uow, ISender sender, IMapper mapper)
    {
        _uow = uow;
        _sender = sender;
        _mapper = mapper;
    ...
    {
        var items = await _uow.Departments.GetAllAsync(ct);
        return Ok(_mapper.Map<IReadOnlyList<DepartmentDto>>(items));
    }
    ...
    public async Task<IActionResult> GetPaged([FromQuery] int page = 1, [FromQuery] int pageSize = 10, [FromQuery] string? search = null, CancellationToken ct = default)
    {
        ...
        var items = await _uow.Departments.GetPagedAsync(page, pageSize, search, ct);
        var total = await _uow.Departments.CountAsync(search, ct);
        var mapped = _mapper.Map<IReadOnlyList<DepartmentDto>>(items);
    ...
    {
        var item = await _uow.Departments.GetByIdAsync(id, ct);
        if (item is null) return NotFound();
        return Ok(_mapper.Map<DepartmentDto>(item));
    }
    ...
    {
        var command = _mapper.Map<CreateDepartment>(request);
        var id = await _sender.Send(command, ct);
        var item = await _uow.Departments.GetByIdAsync(id, ct);
        if (item is null) return NotFound();
        return CreatedAtAction(nameof(GetById), new { id }, _mapper.Map<DepartmentDto>(item));
    }
    ...
    {
        var entity = await _uow.Departments.GetByIdAsync(id, ct);
        if (entity is null) return NotFound();
        var command = _mapper.Map<UpdateDepartment>(request);
